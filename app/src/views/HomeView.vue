  ...
  <div class="h-[calc(100%-80px)] w-full overflow-hidden">
    <div class="flex flex-col h-full bg-[#F4F4F4]">
      ...
      <div class="flex-1 px-4 py-2 relative">
        <div v-if="currentStage" class="w-full h-full relative">
          <div class="relative w-full h-full">
            <template v-for="(stage, stackIndex) in displayStack" :key="stage.id">
              <div
                v-if="stackIndex === 0"
                ...
                <div class="flex-1 p-5 flex flex-col justify-center bg-white relative -mt-4 rounded-t-3xl z-10">
                  <h2 class="font-oswald font-bold text-3xl mb-1 text-black">{{ stage.company }}</h2>
                  <p class="font-inter font-semibold text-gray-500 text-lg mb-4">{{ stage.title }}</p>

                  <div class="flex flex-wrap gap-2">
                    <span v-for="(tag, index) in stage.tags" :key="index" class="px-4 py-1 bg-gray-200 rounded-full font-inter text-sm font-medium text-gray-800">{{ tag }}</span>
                  </div>
                ...
                <div class="absolute inset-0 pointer-events-none z-40">
                  <div v-if="swipeDelta < 0" class="absolute left-6 top-1/2 -translate-y-1/2 flex items-center gap-3 px-6 py-4 rounded-lg bg-red-600/95 text-white font-extrabold text-3xl shadow-2xl border-4 border-white" :style="{ opacity: swipeOpacity, transform: 'translateY(-50%) rotate(-8deg)' }">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18"/><path d="M6 6l12 12"/></svg>
                    NOPE
                  </div>
                  <div v-if="swipeDelta > 0" class="absolute right-6 top-1/2 -translate-y-1/2 flex items-center gap-3 px-6 py-4 rounded-lg bg-emerald-600/95 text-white font-extrabold text-3xl shadow-2xl border-4 border-white" :style="{ opacity: swipeOpacity, transform: 'translateY(-50%) rotate(8deg)' }">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 6L9 17l-5-5"/></svg>
                    LIKE
                  </div>
                </div>
              </div>

              <div v-else class="absolute inset-0 z-20 rounded-3xl overflow-hidden bg-white border border-gray-200" :style="stackCardStyle(stackIndex)">
                <div class="h-[65%] w-full relative bg-gray-300">
                  <img :src="stage.image" class="w-full h-full object-cover" />
                  <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                </div>
...
export default {
  ...
  computed: {
    ...
    swipeDelta() { return this.isSwiping ? (this.touchCurrentX - this.touchStartX) : 0 },
    swipeOpacity() { return this.releaseActive ? 1 : Math.min(Math.abs(this.swipeDelta) / 120, 1) },
    cardStyle() {
    ...
    stackCardStyle() {
      return (stackIndex) => {
        ...
        if (this.releaseActive && stackIndex === 1) {
          return { transform: 'translateY(0px) scale(1)', transition: 'transform 300ms ease', zIndex: 20 - stackIndex }
        }
        return { transform: `translateY(${translateY}px) scale(${scale})`, transition: 'transform 300ms ease', zIndex: 20 - stackIndex }
      }
    }
  },
  methods: {
    nextStage() { this.currentIndex = (this.currentIndex + 1) % this.stages.length },
    prevStage() { this.currentIndex = (this.currentIndex - 1 + this.stages.length) % this.stages.length },
